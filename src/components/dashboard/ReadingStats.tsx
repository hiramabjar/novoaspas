...
import { BookOpen, CheckCircle, BarChart, Activity, Clock, Users } from 'lucide-react'
...
export function ReadingStats({ userId, isAdmin = false }: ReadingStatsProps) {
  const { data: stats, isLoading } = useQuery<ReadingStats>({
    queryKey: ['reading-stats', userId],
    queryFn: async () => {
      const endpoint = userId 
        ? `/api/users/${userId}/stats/reading`
        : '/api/admin/stats/reading'
  ...
  return (
    <Card className="p-6 shadow-lg">
      <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
        ...
        {isAdmin ? 'Estatísticas Gerais de Reading' : 'Seu Progresso em Reading'}
      </h2>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
